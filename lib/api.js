...
const getAuthHeaders = () => {
  ...
  console.log('Getting auth headers with token:', token ? 'Token present' : 'No token')
  return {
...
export const authAPI = {
  ...
  login: async (credentials) => {
    ...
    if (data.data && data.data.token) {
      ...
      console.log('Token stored in localStorage:', data.data.token.substring(0, 20) + '...')
    } else {
      console.warn('No token received in login response. Available fields:', Object.keys(data))
